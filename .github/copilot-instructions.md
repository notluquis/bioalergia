...
**RBAC/ABAC System (Dec 2025):**

- **Architecture**: implemented using CASL + Prisma. Support for both Role-based (RBAC) and Attribute-based (ABAC) access control.
- **Middleware**: `authorize('action', 'Subject')` guards endpoints.
- **Filtering**: `list` endpoints MUST use `accessibleBy(req.ability)` to filter data at the DB level.
- **Admin UI**: New Roles & Permissions management at `src/pages/settings/RolesSettingsPage.tsx`.
- **Frontend**: use `const { can } = useCan()` hook for conditional rendering.
...
- **Daily production balances**: CSV upload acepta `DD/MM/YYYY` (ej. `28/1/2025`), limpia `$`, puntos/comas antes de convertir, y deja status `DRAFT`. En `finanzas/production-balances`, el historial está tras un ícono (panel flotante opcional) y el toggle “Marcar como final” está preparado para pantallas pequeñas.
- **Authorization**:
  - **Backend**: Use `authorize('read', 'Resource')` middleware. ALWAYS pass `req.ability` to service layer list functions for `accessibleBy` filtering.
  - **Frontend**: Protect routes with `<RequirePermission>`. Hide UI elements with `useCan()`.
