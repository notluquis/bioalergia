...
- `server/routes/auth.ts` — Passkey (WebAuthn) + email/password + MFA authentication flows.
- `src/features/auth/pages/LoginPage.tsx` — minimalist passkey-first login UI (biometría → email/password → MFA).
...
- Auth flow: passkey-first (biometría preferred), then email/password, then MFA. Session duration is 24 hours (`server/config.ts`).
...
- `server/config.ts` — session duration (24h), JWT secrets, cookie options.
- `server/routes/auth.ts` — Passkey (WebAuthn) + email/password + MFA authentication.
