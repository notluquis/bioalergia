...
export function TodayMarksList({ marks }: TodayMarksListProps) {
  return (
    <Card className="rounded-3xl shadow-sm" variant="default">
      <Card.Header className="flex flex-col items-start gap-1 p-4 pb-2 md:p-5 md:pb-3">
      ...
      <Card.Content className="flex flex-col gap-2.5 p-4 pt-0 md:gap-3 md:p-5 md:pt-0">
        {marks.length === 0 ? (
          <Card
            className="rounded-2xl border border-dashed border-default-200/70 shadow-none"
            variant="transparent"
          >
            <Card.Content className="px-4 py-6 text-center md:py-8">
          ...
          marks.map((mark) => {
            ...
            return (
              <Card key={mark.id} className="rounded-2xl shadow-none" variant="secondary">
                <Card.Content className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:p-4">
