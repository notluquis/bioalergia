import { Button, Form, Label, ListBox, NumberField, Select, Modal } from "@heroui/react";
...
export function CreateMedicalCertificateModal({ isOpen, onClose }: Props) {
  ...
  const generateMutation = useMutation({
    ...
    onSuccess: async (pdfBlob, variables) => {
      ...
      await queryClient.invalidateQueries({ queryKey: certificatesKeys.medicalLists() });
      onClose();
      form.reset();
    },
    onError: (error) => {
  ...
  const form = useForm({
    defaultValues: {
      patientName: "",
      rut: "",
      birthDate: `${Number(today().slice(0, 4)) - 25}${today().slice(4)}`,
      address: "",
      date: today(),
      diagnosis: "",
      symptoms: "",
                            ...
                            >
                              ...
                              <Select.Popover>
                                <ListBox>
                                  ...
                                  <ListBox.Item
                                    id="estudio"
                                    textValue="Establecimiento educacional"
                                  >
                          ...
                          {(purpose) =>
                            purpose === "otro" && (
                              <form.Field name="purposeDetail">
                                {(field) => (
                                  <TanStackInputField field={field} label="Especificar" />
                                )}
                              </form.Field>
                            )
                          }
