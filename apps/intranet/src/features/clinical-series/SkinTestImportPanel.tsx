...
import {
  ...
  type OneDriveSyncParams,
} from "@/features/onedrive/components/OneDriveAccountManager";
import { useOneDriveStatus } from "@/features/onedrive/hooks/useOneDrive";
...
const STATUS_FILTER_OPTIONS: Array<{ label: string; value: "ALL" | SkinTestImportStatus }> = [
  ...
  { label: "Movidos a ficha", value: "MOVED_TO_RECORD" },
];
...
const STATUS_LABELS: Record<SkinTestImportStatus, string> = {
  ...
  ERROR: "Error",
  IMPORTED: "Importado",
  MOVED_TO_RECORD: "Movido a ficha",
...
export function SkinTestImportPanel() {
  ...
  const oneDrive = useOneDriveStatus();
    ...
    <div className="space-y-4">
      <Surface className="rounded-xl p-4">
        ...
        <div className="mt-4 border-t border-border pt-4">
          <OneDriveAccountManager
            onSyncFolder={(params: OneDriveSyncParams) => void handleSync(false, params)}
            renderSyncActions={(account) => (
              <>
                <Button
                  size="sm"
                  variant="secondary"
                  onPress={() => void handleSync(false, { accountId: account.accountId })}
                  isDisabled={isSyncInProgress}
                >
                  <RefreshCw size={14} />
                  Sync cuenta
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onPress={() => void handleSync(true, { accountId: account.accountId })}
                  isDisabled={isSyncInProgress}
                >
                  <RotateCw size={14} />
                  Releer cuenta
