...
const baseInput: MedicalPrescriptionPdfInput = {
  ...
  medications: [
    { name: "Loratadina 10 mg", dosage: "10 mg, vía oral", frequency: "cada 24 horas", duration: "30 días" },
  ],
  doctorName: "Dr. Test",
...
describe("generateMedicalPrescriptionPdf", () => {
  ...
  it("paginates when many medications overflow", async () => {
    const bytes = await render({
      ...
      medications: Array.from({ length: 8 }, (_, i) => ({
        ...
        duration: "90 días",
        instructions: "Enjuagar la boca después de cada uso. Indicaciones largas para forzar el wrap.",
      })),
    });
    const doc = await PDFDocument.load(bytes);
  ...
  it("embeds a QR + verification code when provided", async () => {
    const qr = await QRCode.toBuffer("https://intranet.bioalergia.cl/verificar/BA-7K2M-9XQ4");
    const bytes = await render({ ...baseInput, qrCodeBuffer: qr, verificationCode: "BA-7K2M-9XQ4" });
    const doc = await PDFDocument.load(bytes);
    expect(doc.getPageCount()).toBe(1);
  });
