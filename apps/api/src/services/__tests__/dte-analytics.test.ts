...
// Characterization tests for the dte-analytics service raw-SQL serialisers +
// the NOT_FOUND DomainError path. The $queryRaw tag is mocked to return canned
// rows; we assert the Number()/dbDateToISO coercion contract the intranet
// client relies on. (The $qb Kysely builder paths are integration-tested
// elsewhere — too brittle to mock the full fluent chain meaningfully.)

const { mockDb } = vi.hoisted(() => ({
  mockDb: { $queryRaw: vi.fn() },
}));
...
describe("getSalesLinkedEvents", () => {
  it("throws NOT_FOUND DomainError when the DTE row is missing", async () => {
    mockDb.$queryRaw.mockResolvedValueOnce([]); // dte lookup returns no rows
  ...
  it("coerces numeric/date fields and returns the linked events list", async () => {
    mockDb.$queryRaw
      ...
      .mockResolvedValueOnce([{ calendarId: "cal-1", eventId: "ev-1", summary: "Consulta" }]);
