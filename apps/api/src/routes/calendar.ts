...
calendarRoutes.get("/calendars/:calendarId/events", requireAuth, async (c) => {
  ...
  if (!calendar) {
    return c.json({ status: "error", message: "Calendar not found" }, 404);
  }
...
calendarRoutes.post("/events/reclassify", requireAuth, async (c) => {
  ...
  c.json({ status: "accepted", jobId, totalEvents: events.length }); // Respond first? No, await issues in Hono.
  ...
  return c.json({ status: "accepted", jobId, totalEvents: events.length });
});
