...
describe("skin test file filter", () => {
  it("accepts multitest, prick test and test cutaneo filename variants", () => {
    ...
    expect(
      isSkinTestCandidateFilename("_panel aeroalérgenos I, + LTP, PROFILINA, LATEX.xlsx")
    ).toBe(true);
    ...
    expect(
      isSkinTestCandidateFilename("MAITE OLIVA GRANDFELDT - ULTITEST 1, 2 , 3 Y Acaros.xlsx")
    ).toBe(true);
  ...
  it("rejects generic clinical and patient spreadsheets", () => {
    ...
    expect(
      isSkinTestCandidateFilename("SOLICITUD ALERGENOS - MULTIAPLICADORES - LANCETAS.xlsx")
    ).toBe(false);
  ...
  it("marks generic skin test templates as non-importable", () => {
    ...
    expect(isSkinTestTemplateFilename("NUEVO PRICK TEST AEROALERGENOS PEDIATRICO.xlsx")).toBe(true);
    expect(isImportableSkinTestFilename("NUEVO PRICK TEST AEROALERGENOS PEDIATRICO.xlsx")).toBe(
      false
    );
    expect(
      isSkinTestTemplateFilename("Copia de PRICK TEST panel aeroalérgenos I(219136).xlsx")
    ).toBe(true);
  ...
  it("keeps patient-specific skin test files importable even if they contain nuevo or copia", () => {
    ...
    expect(isImportableSkinTestFilename("MAIR HASSON GOLUBOFF - TEST CUTANEO AINES.xlsx")).toBe(
      true
    );
    expect(
