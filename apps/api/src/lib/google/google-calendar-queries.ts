...
const EVENT_DATE_SQL = sql<string>`
  COALESCE(
    e.start_date,
    (e.start_date_time AT TIME ZONE ${sql.lit(TIMEZONE)})::date
  )
`;
...
export type CalendarEventFilters = {
  ...
  patientRut?: string;
  search?: string;
  seriesKind?: "PATCH_TEST" | "SKIN_TEST" | "SUBCUTANEOUS_TREATMENT" | "MEDICAL_CONSULTATION";
...
const EVENT_DATE_EXPR = sql`coalesce(e.start_date, (e.start_date_time AT TIME ZONE ${sql.lit(TIMEZONE)}))`;
