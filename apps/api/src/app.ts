...
app.use("/api/orpc/operational-registers/rpc/*", async (c, next) => {
  const { matched, response } = await operationalRegistersORPCHandler.handle(
    createHonoORPCRequest(c),
    {
      prefix: "/api/orpc/operational-registers/rpc",
      context: { hono: c },
    }
  );
