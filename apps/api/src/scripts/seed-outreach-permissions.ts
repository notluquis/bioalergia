...
const TARGET_ROLES = process.env.OUTREACH_SEED_ROLES?.split(",")
  .map((s) => s.trim())
  .filter(Boolean) ?? ["Admin", "Administrador", "GOD"];

async function main() {
  const { db } = await import("@finanzas/db");

  console.log("🌱 Seeding Outreach permissions...");
  ...
  if (roles.length === 0) {
    ...
    console.log("   Lista todos los roles disponibles:");
    const all = await db.role.findMany({ select: { id: true, name: true } });
    for (const r of all) console.log(`   - ${r.id}: ${r.name}`);
    console.log("   Re-ejecuta con OUTREACH_SEED_ROLES='NombreRol' env var.");
    return;
  }
