...
// Permisos del módulo de redes sociales (Fase A). Admin-only por decisión:
// solo roles admin manejan/aprueban publicaciones. Ampliable luego desde la UI
// de roles o con SOCIAL_SEED_ROLES="Marketing,...".
const SUBJECTS = ["SocialPost", "SocialAccount"] as const;
const ACTIONS = ["read", "create", "update", "delete"] as const;
const TARGET_ROLES = process.env.SOCIAL_SEED_ROLES?.split(",")
  .map((s) => s.trim())
  .filter(Boolean) ?? ["SystemAdministrator", "Admin", "Administrador", "Socio"];

async function main() {
  const { db } = await import("@finanzas/db");
  console.log("🌱 Seeding Social permissions (admin-only)...");
  const ids: number[] = [];
