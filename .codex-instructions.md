# Codex AI — finanzas-app Quick Reference
...
Development: npm run dev:full (frontend + backend simultáneo)
...
1. **NEVER use `bg-white`** → usar `bg-base-100` (DaisyUI)
2. **NEVER create mock data** → usar APIs reales existentes
...
⚠️ **CRITICAL**: Database es **PostgreSQL**, NO MySQL. Usar sintaxis PostgreSQL en raw queries (`TO_CHAR`, `EXTRACT`, no `DATE_FORMAT`).
...
## 🔴 Recent Critical Changes (December 2024)
...
- Defined in `server/config.ts` → `parseExcludePatterns()`
- Auto-excludes: "cumpleaños", empty events, date-only events
...
### Daily Production Balances (CSV + UI)
...
```typescript
// ✅ CORRECT - PostgreSQL syntax in raw queries
...
```typescript
// ✅ CORRECT - try-catch with detailed logging
try {
...
```typescript
// ✅ CORRECT - validate amounts before DB insert
...
- `.github/copilot-instructions.md` — Copilot-specific quick guide.
- `.gemini-instructions.md` — Gemini-focused guidance.
...
- `src/features/calendar/api.ts` — Centralized API calls
- `src/features/calendar/hooks/useCalendarEvents.ts` — State + sync management
- `server/routes/calendar-events.ts` — Backend endpoints
- `server/lib/google-calendar-queries.ts` — Raw SQL (PostgreSQL)
- `server/services/calendar.ts` — Sync lock (15min timeout)
- `server/config.ts` — Exclusion patterns
- `server/modules/calendar/parsers.ts` — **NEW**: Amount validation (Int32 + 100M CLP max)
...
```
Use cuando: Debugging complejo, diseño de arquitectura, análisis multi-paso
```
...
- `CLAUDE.md` — Claude AI specific
- `.cursorrules` — Cursor AI specific
