...
2. **NEVER create mock data** → use existing real API endpoints
...
│   │   │   ├── hooks/            # React hooks (useCalendarEvents, etc.)
...
│   │   ├── auth.ts               # Auth endpoints (login, passkey, MFA)
│   │   ├── calendar-events.ts   # Calendar CRUD + sync
│   │   └── transactions.ts      # Transaction preview/import
│   ├── services/                 # Business logic
│   │   └── calendar.ts           # Sync lock (15min timeout)
│   ├── lib/                      # Utilities + helpers
│   │   ├── google-calendar.ts   # Google Calendar API integration
│   │   └── google-calendar-store.ts  # DB upsert logic
│   ├── modules/                  # Domain modules
│   │   └── calendar/parsers.ts  # Event parsing + validation
│   └── config.ts                 # App configuration (sessions, JWT, etc.)
│
├── prisma/
...
## 🔴 Recent Critical Changes (December 2025)
...
// ❌ INCORRECT (MySQL syntax + wrong table)
await prisma.$queryRaw`
...
### 2. Calendar sync is async (no blocking responses)

- `POST /api/calendar/events/sync` returns **202 Accepted** immediately; sync runs in background.
- Frontend polls `GET /api/calendar/events/sync/logs` every 5s; stop after 5 minutes (60 polls).
- Never block >100s to avoid Cloudflare 524.

### 3. Sync timeout is 15 minutes

- `server/services/calendar.ts` marks stale syncs after 15min (previously 5min).
- Keep background jobs and locks aligned with this window.

### 4. Calendar exclusion patterns

- `server/config.ts` → `parseExcludePatterns()` auto-excludes: "cumpleaños", empty summaries, date-only events.
- Extend via `GOOGLE_CALENDAR_EXCLUDE_SUMMARIES`.

### 5. Amount validation (Int32 guardrails)

- `server/modules/calendar/parsers.ts` → `normalizeAmountRaw()`; discard values >2,147,483,647 or >100M CLP.
- Log warnings instead of inserting unsafe amounts.

### 6. Auth: `passwordHash` nullable

- `prisma/schema.prisma`: `passwordHash String?` for passkey-only users; handle nulls in `server/routes/auth.ts`.

### 7. Tabs must use `end: true`

- All layout tabs need `end: true` for correct active state (`CalendarLayout.tsx`, `HRLayout.tsx`, `ServicesLayout.tsx`, `OperationsLayout.tsx`).

### 8. Daily production balances (CSV + UI)

- CSV upload acepta `DD/MM/YYYY` (ej. `28/1/2025`) y limpia `$`, puntos/comas antes de convertir a entero; status por defecto `DRAFT`.
- Página `finanzas/production-balances`: layout más ancho, historial detrás de ícono con panel flotante opcional, y toggle “Marcar como final” es responsive en pantallas pequeñas.
...
// ❌ INCORRECT - inline API calls in component
...
// ❌ INCORRECT - generic error logging
try {
  await syncGoogleCalendar(calendarId);
} catch (error) {
...
| `Unable to fit value X into a 64-bit signed integer`                          | Amount exceeds Int32 max                        | Validate amounts ≤ 2,147,483,647 before DB insert |
...
**DO NOT** change `server/config.ts` → `sessionCookieOptions.maxAge` (24h) without approval.
...
**ALWAYS** validate amounts ≤ Int32 max (2,147,483,647) before database inserts.
...
# 2. Lint
npm run lint
# ✓ No ESLint errors (use --fix if needed)
